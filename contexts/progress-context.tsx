...
import { QuizAttempt, Certificate, UserProgress, TopicProgress } from "@/lib/types"
...
const PROGRESS_KEY = "pep_progress"
...
export function ProgressProvider({ children }: { children: ReactNode }) {
  ...
  const recordQuizAttempt = useCallback((attemptData: Omit<QuizAttempt, "id" | "userId" | "completedAt">) => {
    ...
    const updatedProgress: UserProgress = {
      ...
      totalQuizzesTaken: attemptData.category === "mock-test" 
        ? progress.totalQuizzesTaken 
        : progress.totalQuizzesTaken + 1,
      totalMockTestsTaken: attemptData.category === "mock-test" 
    ...
    updatedProgress.averageScore = allScores.length > 0 
  ...
  const canTakeQuiz = useCallback((quizId: string): boolean => {
    // Premium users can take unlimited quizzes
    if (isPremium) return true
    
    // Free users can only take each quiz once
  ...
  const getTopicProgress = useCallback((category: string): TopicProgress[] => {
    if (!progress) return []
  ...
  const earnCertificate = useCallback((certData: Omit<Certificate, "id" | "userId" | "earnedAt">) => {
    if (!progress || !user) return

    // Check if already has this certificate
    const existing = progress.certificates.find(c => 
      c.quizId === certData.quizId && c.type === certData.type
    )
    if (existing) return
  ...
  const hasCertificate = useCallback((quizId: string): boolean => {
